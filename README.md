...
試験 ＞ 教科 ＞ 分野 ＞ カード の4階層構造で学習データを整理でき、ブラウザ上で直感的に作成・編集・学習が可能です。

## 特徴
- **数式対応**: KaTeX を使用し、問題・回答の両方で LaTeX 形式の数式を描画。
- **4階層管理**: 試験・教科・分野・カードの階層で、膨大な学習データを整理して管理可能。
...
#### 主要プロパティ
- `data`: サーバーから取得した全データ（試験・教科・分野・カードの配列）。
- `currentView`: 現在の画面状態 (`home`, `subjects`, `fields`, `study`, `create-card`)。

#### 主要メソッド
- `fetchData() / saveData()`: サーバーとの通信（JSONの取得・保存）。
- `render()`: `currentView` に基づき、HTMLを動的に生成し KaTeX を適用します。
- `renderFields()`: 分野一覧の描画（新規追加）。
- `renderStudy()`: 学習モードの描画。数式描画と「答えを表示」の状態管理を行います。
- `submitCard()`: カードの新規作成または既存カードの更新を行い、サーバーへ保存します。
- `add/edit/delete[Exam|Subject|Field|Card]()`: 各階層のCRUD操作をフロントエンドで処理し、保存をトリガーします。
